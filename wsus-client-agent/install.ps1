...
# ── 5. Point Windows Update at WSUS + lock down the native UI ────────
...
# Disables the interactive "Check for updates" button (and related
# actions) in Settings > Windows Update, WITHOUT removing the page
# itself from navigation - deliberately chosen over
# SettingsPageVisibility=hide:windowsupdate (an earlier version of this
# script/the tray app used that instead): hiding the whole page also
# hides its Update History sub-page, and NoAutoUpdate alone does NOT
# stop a user from triggering a real scan+install via that button
# manually, which would install updates completely outside this
# agent's own tray flow. This is machine-wide (HKLM), so unlike the
# page-hide it doesn't need to be reapplied per user profile.
Set-ItemProperty -Path $wuKey -Name "SetDisableUXWUAccess" -Value 1 -Type DWord
